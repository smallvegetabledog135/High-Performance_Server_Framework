...
printf("zhujinchengkaishiyunxing");
        ngx_log_error_core(NGX_LOG_NOTICE,0,"%s %P 【masterfather进程】启动并开始运行......!",title,ngx_pid); //设置标题时顺便记录下来进程名，进程id等信息到日志
    }    
    //首先我设置主进程标题---------end
...
printf("woyaochuangjianzijinchengle ");
 ngx_log_error_core(NGX_LOG_ALERT,errno,"kaishichuangjianzijincheng");

    //创建子进程后，父进程的执行流程会返回到这里，子进程不会走进来    
    sigemptyset(&set); //信号屏蔽字为空，表示不屏蔽任何信号
 ngx_log_error_core(NGX_LOG_ALERT,errno,"zijinchengchuangjianwancheng");

    printf("wochaungjianchenggongle");
    for ( ;; ) 
    {
...
{
    int i;
ngx_log_error_core(NGX_LOG_ALERT,errno,"chuangjianle%dgegongzuoxianchen\n",threadnums);

printf("Starting %d worker processes...\n", threadnums); // 输出创建的worker数量 
...
static int ngx_spawn_process(int inum,const char *pprocname)
{
    ...
    case -1: //产生子进程失败
        ngx_log_error_core(NGX_LOG_ALERT,errno,"ngx_spawn_process()fork()产生子进程num=%d,procname=\"%s\"失败!",inum,pprocname);
         printf("Worker process %d created unsuccessfully.\n", inum); // 输出子进程创建成功的信息
        return -1;

    case 0:  //子进程分支
 ngx_log_error_core(NGX_LOG_ALERT,errno,"chanshengzijincheng%dchenggong\n",inum);

printf("Worker process %d created successfully.\n", inum); // 输出子进程创建成功的信息
 ...
 ngx_log_error_core(NGX_LOG_ALERT,errno,"fujinchengfenzhi");
...
static void ngx_worker_process_cycle(int inum,const char *pprocname) 
{
 ngx_log_error_core(NGX_LOG_ALERT,errno,"jinrugongzuoxianchenghanshu");

printf("Worker process %d is entering its cycle.\n", inum); // 输出worker进程进入循环的信息
    //设置一下变量
    ...
    ngx_log_error_core(NGX_LOG_NOTICE,0,"%s %P 【workerchild进程】启动并开始运行......!",pprocname,ngx_pid); //设置标题时顺便记录下来进程名，进程id等信息到日志
