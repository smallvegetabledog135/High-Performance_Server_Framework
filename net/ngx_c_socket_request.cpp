...
{  
ngx_log_stderr(0,"kaishifenxishuju");
ngx_log_error_core(NGX_LOG_INFO,errno,"kaishifenxishuju2"); 
    bool isflood = false; //是否flood攻击；

// 预设的数据包（根据之前的 Python 客户端构造）  
    ...
    if(reco <= 0)  
    {
ngx_log_stderr(0, "recvproc failed or no data received, reco: %zd", reco);         
 return;//该处理的上边这个recvproc()函数处理过了，这里<=0是直接return        
    }
ngx_log_stderr(0, "Received Raw Data:");   
    for (ssize_t i = 0; i < reco; i++) {  
        ngx_log_stderr(0, "%02x", static_cast<unsigned char>(pConn->precvbuf[i])); // 以十六进制显示  
ngx_log_stderr(0, "\n"); // 结束一行   
 }  
    ...
    {        ngx_log_error_core(NGX_LOG_INFO,errno,"shoudaobaotou");
        if(reco == m_iLenPkgHeader)//正好收到完整包头，这里拆解包头
        {
ngx_log_stderr(0,"wanzhengbaotou,kaishichaijie");
ngx_log_error_core(NGX_LOG_INFO,errno,"wanzhengbaotou,kaishichaijie");
   
            ngx_wait_request_handler_proc_p1(pConn,isflood); //那就调用专门针对包头处理完整的函数去处理把。
        }
        else
		{
ngx_log_error_core(NGX_LOG_INFO,errno,"baotoubuwanzheng");
    ...
    {
ngx_log_error_core(NGX_LOG_INFO,errno,"baotoubuwanzhengjixushou");       
...
{    
ngx_log_stderr(0,"baochuli");
ngx_log_error_core(NGX_LOG_INFO,errno,"BAOCHULI");
    ...
    {ngx_log_stderr(0,"hefabao");
ngx_log_error_core(NGX_LOG_INFO,errno,"hefabao");
        ...
        if(e_pkgLen == m_iLenPkgHeader)
        {
ngx_log_stderr(0,"panduanxiaoxitouchangduhebaochangdu");
ngx_log_error_core(NGX_LOG_INFO,errno,"panduanxiaoxitouchangduhebaochangdu");
        ...
        {
ngx_log_stderr(0,"changduduibushang");
ngx_log_error_core(NGX_LOG_INFO,errno,"changduduibushang");
...
ngx_log_stderr(0,"biejinla,changdufenbiewei%dhe%d",e_pkgLen,m_iLenPkgHeader);
ngx_log_error_core(NGX_LOG_INFO,errno,"biejinlai,changdufenbiewei%dhe%d",e_pkgLen,m_iLenPkgHeader);
...
void CSocekt::ngx_wait_request_handler_proc_plast(lpngx_connection_t pConn,bool &isflood)
{
    ...
    {
ngx_log_stderr(0,"ruduilie");
ngx_log_error_core(NGX_LOG_INFO,errno,"RUDUILIE");
     
   g_threadpool.inMsgRecvQueueAndSignal(pConn->precvMemPointer); //入消息队列并触发线程处理消息
    }
    else
    {
ngx_log_stderr(0,"yougongjiqingxiang");
ngx_log_error_core(NGX_LOG_INFO,errno,"YOUGONGJIQINGXIANG");
