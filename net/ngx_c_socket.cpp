...
ngx_log_error_core(NGX_LOG_INFO,errno,"JINRUCHULISHIJIANHANSHU,TIMERSHI%d",timer);
   
 int events = epoll_wait(m_epollhandle,m_events,NGX_MAX_EVENTS,timer);    
    
ngx_log_error_core(NGX_LOG_INFO,errno,"EVENT");
    ...
    //uintptr_t          instance;
    uint32_t           revents;
ngx_log_error_core(NGX_LOG_INFO,errno,"zoudaoxunhuanqianle ");

    for(int i = 0; i < events; ++i)    //遍历本次epoll_wait返回的所有事件，注意events才是返回的实际事件数量
    {
ngx_log_error_core(NGX_LOG_INFO,errno,"iEVENT",i);
        ...
        {
          ngx_log_error_core(NGX_LOG_INFO,errno,"du");
